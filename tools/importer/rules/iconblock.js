...
const creativityForAllIconBlock = (main, document) => {
  const iconBlockCreation = (container, type) => {
    ...
    if (imageLink && titleElement && descriptionElement.length && linkElement) {
      ...
      const parent = container.closest('.xf')
        ? container.closest('.xf')
        : container;
  ...
  const containers = [];
  [...main.querySelectorAll('.dexter-FlexContainer-Items')].forEach(
    (c) => {
...
const expressIconBlock = (main, document) => {
  const el = main.querySelector('#express');

  if (el && !el.querySelector('[daa-lh="mobile-marquee"]')) {
...
export default function createIconBlock(main, document) {
  expressIconBlock(main, document);
  creativityForAllIconBlock(main, document);
}
